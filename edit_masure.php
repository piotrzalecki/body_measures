...
if(isset($_GET["id"])){
  ...
  $id = $_GET["id"];
  $stmt1 = mysqli_prepare($connection, "SELECT date, weight, hips, belly, waist, leg,calf, chest, forearm, biceps, neck FROM {$_SESSION['user_db']} WHERE id =?");
  mysqli_stmt_bind_param($stmt1,'i',$id);

  if (!$stmt1->execute()) {
    echo "Execute failed: (" . $stmt->errno . ") " . $stmt->error;}
    mysqli_stmt_execute($stmt1);
    mysqli_stmt_bind_result($stmt1, $db_date, $db_weight, $db_hips, $db_belly, $db_waist, $db_thigh, $db_calf, $db_chest, $db_forearm, $db_biceps, $db_neck);
  ...
  if(isset($_POST["submit"])){
    ...
    mysqli_stmt_bind_param($stmt2,'siiiiiiiiiii',$date,$weight,$hips,$belly,$waist,$thigh,$calf,$chest,$forearm,$biceps,$neck,$id);
    $stmt2 = mysqli_prepare($connection, "UPDATE {$_SESSION['user_db']} SET date=? , weight = ?, hips= ? , belly = ?, waist = ?, leg = ?, calf = ?, chest = ?, forearm = ?, biceps = ?, neck = ? WHERE id=?");
...
<section>
  <div class="section_wraper">
    ...
    <h1>Edit Masure</h1>
    <form class="add_masure" id="addMeasureForm" action ="" method="post" >
